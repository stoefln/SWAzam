...
public class ClientAgent extends Agent {
	...
	protected void setup() {
		...
		// 1) look for an agent which has registered as "SWAzamServer" every 10 seconds (= wait till a server is online)
		...
		addBehaviour(new OneShotBehaviour(){
			...
			public void action() {
				...
				if(server != null){
					...
					log("Search request with fingerprint \""+searchFingerprint+"\" sent to server "+server.getName());
				}else{
	...
	private class ResponseReceiver extends CyclicBehaviour{
		...
		public void action() {
			...
			if (reply != null) {
				...
				if(reply.getPerformative() == ACLMessage.CONFIRM) { // music found
					...
					log("music found! "+searchResponse.toString());
				} else {
					// TODO: display message in GUI
					log("nothing found. reason: " + reply.getContent());
				}
			} else {
				block();
			}
